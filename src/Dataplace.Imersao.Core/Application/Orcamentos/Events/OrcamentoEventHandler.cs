...
namespace Dataplace.Imersao.Core.Application.Orcamentos.Events
{
    public class OrcamentoEventHandler :
        ...
        INotificationHandler<DataDePrevisaoDeEntregaAtribuidaAoOrcamentoEvent>,
        INotificationHandler<DataDePrevisaoDeEntregaRemovidaAoOrcamentoEvent>
    {
        #region fields
        ...
        public async Task Handle(DataDePrevisaoDeEntregaAtribuidaAoOrcamentoEvent notification, CancellationToken cancellationToken)
        {
            var cdEmpresa = _environmentService.GetCdEmpresa();
            var cdFilial = _environmentService.GetCdFilial();
            var origem = notification.GetType().Name;
            var categoria = "Alteração";
            var tipo = "Linha";
            var descricao = "Atribuição de data de previsão de entrega do Orçamento";
            var detalhe = JsonConvert.SerializeObject(notification);
            _symphonyLogService.Register(cdEmpresa, cdFilial, origem, categoria, tipo, descricao, detalhe);
        }

        public async Task Handle(DataDePrevisaoDeEntregaRemovidaAoOrcamentoEvent notification, CancellationToken cancellationToken)
        {
            var cdEmpresa = _environmentService.GetCdEmpresa();
            var cdFilial = _environmentService.GetCdFilial();
            var origem = notification.GetType().Name;
            var categoria = "Alteração";
            var tipo = "Linha";
            var descricao = "Remoção de data de previsão de entrega do Orçamento";
            var detalhe = JsonConvert.SerializeObject(notification);
            _symphonyLogService.Register(cdEmpresa, cdFilial, origem, categoria, tipo, descricao, detalhe);
        }
