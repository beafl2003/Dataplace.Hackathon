...
namespace Dataplace.Imersao.Core.Infra.Data.Repositories
{
    public class OrcamentoRepository : IOrcamentoRepository
    {
        ...
        {
            //OrcamentoItem.QtdDiaPrevEntrega, OrcamentoItem.dtpreventrega
            var sql = @"
            INSERT INTO 
               OrcamentoItem(CdEmpresa, CdFilial, NumOrcamento, qtdproduto, stitem, tpregistro, cdproduto, vlvenda, percaltpreco, vlcalculado, QtdDiaPrevEntrega, dtpreventrega)
            VALUES(?,?,?,?,?,?,?,?,?,?,?,?);
            ...
            var result = _dataAccess.Connection.ExecuteScalar<int?>(sql,
                new
                {
                    ...
                    QtdDiaPrevEntrega = entity.PrevisaoEntrega?.Dias,
                    dtpreventrega = entity.PrevisaoEntrega?.Data
                },
                 transaction: _dataAccess.Transaction);
        ...
        public bool AtualizarItem(OrcamentoItem entity)
        {
            ...
            UPDATE 
                OrcamentoItem
            SET
                qtdproduto= ?, stitem= ?, tpregistro= ?, cdproduto= ?, vlvenda= ?, percaltpreco= ?, vlcalculado= ?, QtdDiaPrevEntrega= ?, dtpreventrega= ?
            WHERE CdEmpresa = ?
                AND CdFilial = ?
                AND NumOrcamento = ?
                AND Seq = ?
        ...
        //{
        //    var sql = @"
        //    SELECT Seq, CdEmpresa, CdFilial, NumOrcamento, qtdproduto, stitem, qtdproduto * vlvenda as Total, tpregistro as TpProduto, cdproduto, vlvenda as PrecoTabela, vlcalculado as PrecoVenda, percaltpreco as PercAltPreco,
        //            OrcamentoItem.QtdDiaPrevEntrega, OrcamentoItem.dtpreventrega
        //        FROM  
        //    OrcamentoItem
        //        WHERE CdEmpresa = ? AND CdFilial = ? AND NumOrcamento = ? AND Seq = ?
        //    ";
        //    var items = _dataAccess.Connection.Query<OrcamentoItem, OrcamentoProduto, OrcamentoItemPreco, OrcamentoPrevisaoEntrega, OrcamentoItem>(sql,
        //        (item, produto, preco, entrega) => {
        ...
        //            if (entrega != null)
        //                item.DefinirPrevisaoEntrega(entrega);
        ...
        //            Seq = seq
        //        },
        //        splitOn: "TpProduto, PrecoTabela, QtdDiaPrevEntrega",
        //        transaction: _dataAccess.Transaction);

        //    return items.FirstOrDefault();
        //}
        private IEnumerable<OrcamentoItem> ObterItems(string cdEmpresa, string cdFilial, int numOrcamento)
        {
            var sql = @"
            SELECT Seq, CdEmpresa, CdFilial, NumOrcamento, qtdproduto, stitem, qtdproduto * vlvenda as Total, tpregistro as TpProduto, cdproduto, vlvenda as PrecoTabela, vlcalculado as PrecoVenda, percaltpreco as PercAltPreco,
                    OrcamentoItem.QtdDiaPrevEntrega, OrcamentoItem.dtpreventrega
